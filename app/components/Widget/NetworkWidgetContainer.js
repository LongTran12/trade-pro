...
const NetworkWidgetContainer = () => {
    ...
    useEffect(() => {
        const getInfo = async () => {
            let member = new web3Public.eth.Contract(config.memberAbi, config.memberAddress);
            let memberInfo = await member.methods.infoMember(address).call();
            let linaData = new web3Public.eth.Contract(config.linaDataAbi, config.linaData);
            let refs = []
            console.log('membetr', memberInfo)
            for (let i = 0; i < memberInfo.refs.length; i++) {
                let agencyInfo = await linaData.methods.getUserInfo(memberInfo.refs[i]).call()
                refs.push({
                    ...
                    title:
                        <div className="ref_title">
                            ...
                            <div className="ref_stat">
                                <span className="ref_user hidden-mobile">[Total Invest: {agencyInfo.totalInvest / (10 ** 18)}</span>
                                <span className="hidden-mobile">|</span>
                                <span className="ref_commission">Total Token: <span>{agencyInfo.totalToken / (10 ** 18)}</span> LINA| Sale: {agencyInfo.totalSales / 10 ** 18}<span className="hidden-mobile">]</span></span>
                            </div>
                        </div>,
    ...
    const onLoadData = async (treeNode) => {
        ...
        let member = new web3Public.eth.Contract(config.memberAbi, config.memberAddress)
        let memberInfo = await member.methods.infoMember(treeNode.props.name).call()
        ...
        for (let i = 0; i < memberInfo.refs.length; i++) {
            ...
            treeNode.props.dataRef.children.push({
                ...
                title:
                    <div className="ref_title">
                        <div className="ref_name">
                            <span className="level__ref">F{treeNode.props.eventKey.replace(/[^-]/g, "").length + 2}</span>
                            <span className="address__ref">{memberInfo.refs[i]}</span>
                        </div>
                        <div className="ref_stat">
                            <span className="ref_user hidden-mobile">[Total Invest: {agencyInfo.totalInvest / (10 ** 18)}</span>
    ...
    .ant-tree {
        ...
        &>li{
            &>.ant-tree-node-content-wrapper{
                ...
                .level__ref, .address__ref,.ref_stat span {
                    color:#fff;
                }
            }
            &>ul{
